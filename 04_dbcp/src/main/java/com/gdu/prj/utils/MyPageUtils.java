...
public class MyPageUtils {
  ...
  private int total;    // 전체 게시글 개수                      (DB 에서 구한다.) 
  ...
  public void setPaging(int total, int display, int page) {
    ...
    // - totalPage와 endPage 중에서 작은 값을 써야함. (메뉴 수정)
    totalPage = (int)Math.min(totalPage, endPage);
    beginPage = ((page - 1) / pagePerBlock) * pagePerBlock + 1;
  ...
  public String getPaging(String requestURI, String sort, int display) {
    ...
    // <
    if(beginPage == 1) {
      builder.append("<div class=\"dont-click\">&lt;</div>"); // 웹화면에 표시되므로 엔티티 코드가 더 안전함.
    } else {
